...
import { getServerConfig } from './utils.js'

function convertToClientConfig(serverConfig: MCPServerConfig): MCPClientConfig {
  return {
  ...
  for (const serverConfig of serverConfigData.mcp_servers) {
    if (serverConfig.enabled) {
      try {
        ...
        console.error(`服务器 ${serverConfig.server_name} 连接失败:`, error);
      }
    }
  }
